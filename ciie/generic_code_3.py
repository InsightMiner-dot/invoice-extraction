...
# 0. Utility Functions & Data Loading
...
def load_master_spend():
    ...
    Forces invoice numbers and keys to remain as strings to prevent float conversion.
    """
    try:
        current_script_dir = os.path.dirname(os.path.abspath(__file__))
        config_dir = os.path.join(current_script_dir, "..", "config")
...
async def process_batch_concurrently(files_chunk_bytes, client, deployment, custom_fields_dict, standard_aliases_dict, max_pages, dpi):
    """Processes a chunk of files simultaneously using asyncio.gather."""
    tasks = []
    for file_bytes in files_chunk_bytes:
        task = extract_invoice_data_async(
            client, deployment, file_bytes, custom_fields_dict, standard_aliases_dict, max_pages, dpi
        )
        tasks.append(task)
    
    # return_exceptions=True ensures that one bad PDF doesn't crash the entire async batch
...
def rebuild_excel_from_state(custom_cols):
    """Dynamically rebuilds the Excel buffer so manual QA edits are reflected in the download."""
    wb, ws_details, ws_qc = setup_excel_workbook(custom_cols)
    
    if st.session_state.extraction_details:
        for row_dict in st.session_state.extraction_details:
            ws_details.append(list(row_dict.values()))
            
    if st.session_state.extraction_summary:
        for row_dict in st.session_state.extraction_summary:
            ws_qc.append(list(row_dict.values()))
    ...
    try:
        for i in range(0, total_files, CONCURRENCY_LIMIT):
            ...
            for file in current_chunk_files:
                ...
                st.session_state.uploaded_files_cache[file.name] = b
            ...
            for idx, result in enumerate(batch_results):
                filename = current_chunk_files[idx].name
                file_bytes = chunk_bytes[idx]
                file_proc_time = round(time.time() - chunk_start_time, 2) 
                
                # GRACEFUL FAILURE HANDLING - Records the failure but allows the batch to continue
                if isinstance(result, Exception):
                    current_run_summary.append({
                        "File Name": filename, "Raw Vendor Name": "ERROR", "Clean Vendor Name": "ERROR", "Invoice Number": "ERROR", 
                    ...
                    current_run_audit.append({
                        "file_name": filename, "raw_vendor": "ERROR", "clean_vendor": "ERROR", "invoice_number": "ERROR", "origin": "ERROR", "destination": "ERROR",
                ...
                if not extracted_document.invoices:
                    ws_qc.append([filename, "N/A", "N/A", "N/A", "N/A", "N/A", "FAIL - NO DATA", "0 Invoices Found in PDF", 0.0, 0.0, 0.0, "No", 0.0, 0.0, "False", ""])
                    current_run_summary.append({
                        "File Name": filename, "Raw Vendor Name": "N/A", "Clean Vendor Name": "N/A", "Invoice Number": "N/A", "Origin": "N/A", "Destination": "N/A", 
                    ...
                    current_run_audit.append({
                        "file_name": filename, "raw_vendor": "N/A", "clean_vendor": "N/A", "invoice_number": "N/A", "origin": "N/A", "destination": "N/A",
                        "final_origin": "", "final_destination": "", "uoms": "N/A", "in_master": "No", "master_spend": 0.0, "master_variance": 0.0,
                ...
                for extracted_data in extracted_document.invoices:
                    ...
                    # Extract parent folder name from relative file path
                    file_name_path = filename
                    parent_folder = os.path.basename(os.path.dirname(file_name_path))
                    
                    if parent_folder and parent_folder.strip():
                        clean_vendor = parent_folder.strip()
                    ...
                    if clean_vendor and extracted_data.invoice_number:
                        ...
                        if master_key in master_spend_dict:
                            ...
                            # COMPARE MASTER AGAINST CALCULATED SUM
                    ...
                    def create_row_dict(page_num, material, desc, qty, uom, uom_conf, price, line_total, line_orig=None, line_dest=None, opt_warn=""):
                        ...
                        row_data = {
                            ...
                            "Raw Vendor Name": raw_vendor, "Clean Vendor Name": clean_vendor,
                    ...
                    summary_dict = {
                        ...
                        "Extracted Total": extracted_data.total_amount, "Calculated Sum": total_calculated, "Variance": variance, 
                    ...
                    current_run_audit.append({
                        ...
                        "uoms": uom_string,
                        "in_master": in_master,
                        "master_spend": master_spend,
                        "master_variance": master_variance,
                        "status": status,
            ...
            if batch_num_display < total_files:
        ...
        # THE COMMIT BLOCK - Ensures all successful extractions are saved regardless of crashes
        excel_buffer = io.BytesIO()
        wb.save(excel_buffer)
        excel_buffer.seek(0)
...
with st.sidebar:
    ...
    with st.expander("Processing Configuration", expanded=False):
        ...
        st.divider()
        config_batch_size = st.slider(
            "Concurrency Limit (Batch Size)", 
            min_value=1, 
            max_value=20, 
            value=4, 
            help="Determines how many files to process at the exact same time."
        )
        st.caption("💡 Tip: Reduce batch size if processing large PDFs to avoid local memory crashes. Increase for smaller files to maximize speed.")
    ...
    vendor_mode = st.radio("Select Vendor Mode:", ["Multiple Vendors", "Single Vendor"], help="If all uploaded invoices belong to one supplier, select Single Vendor to easily standardize the name.")
    ...
    st.write("Upload your mixed batch of invoices or select a master folder here.")
    
    # NEW DIRECTORY UPLOADER LOGIC
    uploaded_files = st.file_uploader(
        "Upload PDF Documents or Directory", 
        type=["pdf"], 
        accept_multiple_files="directory", 
        help="Select a folder. If files are inside subfolders (e.g., /Vendor A/invoice.pdf), the subfolder name will automatically become the Clean Vendor Name."
    )
...
with tab_extract_view:
    if uploaded_files:
        ...
        with st.expander("🔍 Click to Review Uploaded Documents", expanded=False):
                    ...
                    with col:
                        st.markdown(
                            ...
                            <div style='white-space: nowrap; overflow: hidden; 
                                        ...
                                        margin-bottom: 5px;' title='{file.name}'>
                                <b>{file.name}</b>
                            </div>
                            """,
...
with tab_batch_qa:
    ...
    if batch_df.empty:
        ...
        if 'uoms' not in batch_df.columns:
            batch_df['uoms'] = 'N/A'
        if 'in_master' not in batch_df.columns:
            batch_df['in_master'] = 'No'
        ...
        if b_selected_vendors:
            b_filtered_summary = b_vendor_summary[b_vendor_summary['Raw Vendor'].isin(b_selected_vendors)]
        else:
        ...
        st.write("Manually enter or correct the **Clean Supplier Name**. Once saved, the system will instantly check the **Master Spend DB** for matches and dynamically rebuild your final Excel output.")
        ...
        if st.button("💾 Save Clean Vendor Names & Sync with Master DB", type="primary"):
            ...
            for index, row in edited_vendors.iterrows():
                ...
                for detail in st.session_state.extraction_details:
                    if detail['File Name'] == fname:
                        detail['Clean Vendor Name'] = new_clean_name
        ...
        st.write("If an invoice is missing an Origin or Destination, you can manually approve them here.")
        ...
        if not current_batch_issues.empty:
            ...
            edited_routes = st.data_editor(
                ...
                column_config={
                    ...
                    "raw_vendor": st.column_config.TextColumn("Raw Vendor", disabled=True),
        ...
        st.write("Run a final comparison against the Master Spend Database to ensure all manual cleanups and overrides are accurately reflected, then view and download the final QC report.")
