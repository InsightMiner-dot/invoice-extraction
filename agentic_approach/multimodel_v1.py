...
import pandas as pd
import instructor
from openai import AsyncAzureOpenAI # NEW: Async OpenAI Client
from pydantic import BaseModel, Field
from typing import List, Optional
from azure.core.credentials import AzureKeyCredential
from azure.ai.documentintelligence.aio import DocumentIntelligenceClient # NEW: Async DI Client
from azure.ai.documentintelligence.models import AnalyzeDocumentRequest
from dotenv import load_dotenv

load_dotenv()

# --- 1. SCHEMA (Stable, Clean Structure) ---
...
async def process_single_invoice_async(file_path: str, di_client: DocumentIntelligenceClient, ai_client, semaphore: asyncio.Semaphore):
    """Processes a single invoice asynchronously. Wrapped in a semaphore to prevent API rate limiting."""
    
    # Wait for an open slot in the semaphore before proceeding
    async with semaphore:
        ...
        try:
            ...
            # Step 5: COLUMN REORDERING AND RENAMING (Updated Headers)
            base_order = [
            ...
            # UPDATED: Removed "(Full address)" from all specific targets
            rename_map = {
                ...
                "remit_to": "Remit To",            # Changed
                "shipper": "Shipper",              # Changed
                "bill_to": "Bill To",              # Changed
            ...
            return None # Return None if this specific file fails, so the batch doesn't crash
...
async def process_folder_batch(input_folder: str, output_csv: str):
    ...
    # 3. Setup Concurrency Limit (Crucial to avoid API Rate Limits)
    # This ensures only 5 documents are being processed simultaneously. 
    # If you have high quota limits in Azure, you can increase this to 10 or 20.
    semaphore = asyncio.Semaphore(5) 
    
    # 4. Create tasks for all files
    tasks = [
        process_single_invoice_async(file_path, di_client, ai_client, semaphore)
        for file_path in files_to_process
    ]
    
    # 5. Run all tasks concurrently and gather results
    results = await asyncio.gather(*tasks)
    
    # 6. Filter out any failed documents (which returned None)
    valid_dfs = [df for df in results if df is not None]
    
    # 7. Merge all DataFrames and Export
    if valid_dfs:
...
if __name__ == "__main__":
    ...
    INPUT_DIRECTORY = "./invoice_batch"  # Create this folder and put your test PDFs inside
