...
# ==============================================================
# 1. Define Data Schema (ADDED SUGGESTED ADDRESS CATCH-ALL)
# ==============================================================
...
class InvoiceData(BaseModel):
    ...
    # ---> NEW CATCH-ALL FIELD <---
    suggested_address: Optional[str] = Field(None, description="CATCH-ALL: If you find a full physical address on the invoice, but it does NOT have a clear label (like 'Bill To', 'Origin', etc.), extract it here. If there are multiple orphan addresses, separate them with a semicolon. Leave null if no extra addresses exist.")
...
def extract_invoice_data(pdf_path: str) -> InvoiceData:
    ...
    content_array = [{"type": "text", "text": "Extract data from this multi-page invoice. Use the 'suggested_address' field for any addresses that lack proper labels. Evaluate confidence ('High', 'Medium', 'Low'). Note page numbers."}]
...
def setup_excel_workbook():
    ...
    # ---> ADDED 'Suggested Address' TO HEADERS <---
    details_headers = [
        "File Name", "Page #", "Vendor Name", "Vendor Address", "Bill To", "Remit To",
        "Origin", "Destination", "Suggested Address", "Invoice Number", "Date", "Currency", 
    ...
    for filename in os.listdir(input_folder):
        if filename.lower().endswith(".pdf"):
            ...
            try:
                ...
                # ---> NEW FLAG TRIGGER: Warn user if an untagged address was found <---    
                if extracted_data.suggested_address:
                    review_reasons.append("Orphan Address Found")
                ...
                # ---> UPDATED: Inject suggested_address into the row <---
                def create_row(page_num, material, desc, qty, uom, uom_conf, price, line_total):
                    return [
                        ...
                        extracted_data.origin, extracted_data.destination, extracted_data.suggested_address, 
                        extracted_data.invoice_number, extracted_data.date, extracted_data.currency, 
                ...
                error_row_details = [filename] + [""] * 24 + ["FAIL - CRITICAL ERROR", error_msg]
