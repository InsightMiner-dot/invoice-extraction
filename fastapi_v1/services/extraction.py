    ...
    for extracted_data in extracted_doc.invoices:
        ...
        def create_ui_row(page_num, material, desc, qty, uom, price, line_total):
            row = {
                ...
                "Material": material, "Description": desc, "Qty": qty, "UOM": uom, "Price": price, 
                "Line Total": line_total, "Inv# Conf": extracted_data.invoice_number_confidence, 
                "Total Conf": extracted_data.total_amount_confidence, "Variance": variance, 
                "Proc Time": f"{file_proc_time}s"
            }
...
def generate_excel_from_db(batch_id: str, custom_cols: list):
    ...
    ws_qc.append(["File Name", "Vendor Name", "Invoice Number", "Origin", "Destination", "Status", "Reason for Review", "Extracted Total", "Calculated Sum", "Variance"])
