...
def pdf_to_base64_images(pdf_path: str) -> List[str]:
    ...
    for page_num in range(len(doc)):
        page = doc[page_num]
        pix = page.get_pixmap(dpi=300) # Keep this at 300 for better accuracy
...
def setup_excel_workbook():
    ...
    details_headers = [
        ...
        "Status", "Reason for Review" # Saftey Net columns
    ]
    ...
    for filename in os.listdir(input_folder):
        if filename.lower().endswith(".pdf"):
            ...
            try:
                ...
                # ---> THE FIX IS HERE <---
                # If there are no line items, pass 'None' for all item fields so they stay blank!
                if len(extracted_data.line_items) == 0:
                    ws_details.append(create_row(
                        None, None, None, None, None, None, None, 0.0
                    ))
                else:
                    # Otherwise, write the actual line items normally
                    for item in extracted_data.line_items:
                        ws_details.append(create_row(
